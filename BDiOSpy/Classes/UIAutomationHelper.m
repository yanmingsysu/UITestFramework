...
static UIAApplication * BD_frontMostApp(id self, SEL _cmd)
{
...
static id BD_firstWithPredicate(id self, SEL _cmd, id predicate)
{
...
static void FixReactivateApp(void)
{
    ...
    if ([processInfo respondsToSelector:@selector(isOperatingSystemAtLeastVersion:)] && [processInfo isOperatingSystemAtLeastVersion:(NSOperatingSystemVersion){9, 0, 0}]) {
        // Workaround bug in iOS 9: https://github.com/BD-framework/BD/issues/703
        ...
        method_setImplementation(frontMostApp, (IMP)BD_frontMostApp);
        ...
        method_setImplementation(firstWithPredicate, (IMP)BD_firstWithPredicate);
    }
}
...
// Inspired by:  https://github.com/jamesjn/BD/tree/acknowledge-location-alert
- (BOOL)acknowledgeSystemAlertWithIndex:(NSUInteger)index {
    UIAAlert *alert = [self currentSystemAlert];
    ...
    if (![alert isKindOfClass:[self nilElementClass]] && [self _alertIsValidAndVisible:alert] && isIndexInRange) {
        ...
        while ([self _alertIsValidAndVisible:alert]) {
            ...
            RunLoopRunInModeRelativeToAnimationSpeed(UIApplicationCurrentRunMode, 0.1, false);
        }
        // Wait for alert dismissial animation.
        RunLoopRunInModeRelativeToAnimationSpeed(UIApplicationCurrentRunMode, 0.4, false);
        return YES;
    }
    return NO;
}
